...
A1 = "/content/my_spc/slow1.py"
A2 = "/content/my_spc/slow2.py"
A3 = "/content/my_spc/slow3.py"
...
def scale_to_log(val, min_val, max_val, scale_min, scale_max):
    ...
    return ((log_val - log_min_val) / (log_max_val - log_min_val)) * (
        scale_max - scale_min
    ) + scale_min
...
def get_track_info_from_playlist(url):
    ...
    valid_filename = "".join(
        c for c in playlist_name if c.isalnum() or c in (" ", ".")
    ).rstrip()
    ...
    with open(csv_file_path, mode="w", newline="") as file:
        ...
        while True:
            ...
            for i, item in enumerate(
                tqdm(tracks, total=total_tracks, initial=1, bar_format="<{elapsed}>")
            ):

                total_tracks += 1
                indice1 = total_tracks
                ...
                formatted_danceability = (
                    f"Dnc {danceability:02d}"
                    if danceability < 10
                    else f"Dnc {danceability}"
                )
                energy = audio_features["energy"]
                energy = round(audio_features["energy"] * 10)
                formatted_energy = (
                    f"Nrg {energy:02d}" if energy < 10 else f"Nrg {energy}"
                )
                ...
                for i in range(index, len(scripts)):
                    ...
                    if returncode == 0:
                        ...
                        print(
                            f"Subprocess {scripts[i]} failed with return code {returncode}, trying next script"
                        )
